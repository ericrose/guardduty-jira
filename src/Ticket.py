...
class Ticket(BaseClass):
    ...
    def __init__(self, data, is_lambda):
        ...
        self.guard_duty_link = self.get_guard_duty_link(data)
    ...
    def create(self):
        try:
            ...
            create_ticket_url = '{0}/rest/api/2/issue/'.format(self.CONFIGS['endpoint'])
    ...
    def exists(_self, data):
        try:
            ...
            jira_url = _self.CONFIGS['endpoint']
            project = _self.CONFIGS['jira_project']
            search_ticket_url = "{0}/rest/api/2/search?jql=project={1}+and+text~'guardduty+{2}'".format(jira_url, project, incident_number)
    ...
    def get_guard_duty_link(self, data):
        return "https://console.aws.amazon.com/guardduty/home?region={0}#/findings?search=id%3D{1}".format(data['region'],data['id'])

    @classmethod
    def get_jira_auth(_self):
        return HTTPBasicAuth(_self.CONFIGS['user'], _self.CONFIGS['password'])
